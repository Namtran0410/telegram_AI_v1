...
export class ServiceAiVideo {
    ...
    async actionAiVideoSelection(){
        ...
        this.bot.on("message:text", async(c)=> {
            const res = await Request.aiVideo.requestCutVideo(c.session.file_id, c.session.duration, +c.msg.text)
        ...
        /** Get video from telegram */
            ...
            if (!response.ok) throw new Error("Không thể tải file từ server Telegram")
        ...
        /** Cut video by time */
            const cutFile = await this.plugginVideo.cutVideoBySecond(localFilePath, outputDir, Number(c.msg.text))
        ...
        /** Deal with upload information*/
            ...
            await c.reply(`${res.message}, your star balance now is: ${remainStar} ⭐`, {reply_markup: UI.menuKeyboard.btnMenu()})
        })
    }
}
...
export class PlugginVideo {
    ...
    async cutVideoBySecond(inputPath: string, outputDir: string, time: number): Promise<string[]> {
        ...
        while (startTime < totalDuration) {
            ...
            const outputPath = path.join(outputDir, `part_${index}_${Date.now()}${ext}`);
            ...
            await new Promise<void>((resolve, reject) => {
                ffmpeg(inputPath)
                    ...
                    .output(outputPath)
                    .on("end", () => {
                        console.log(`Đã cắt xong đoạn ${index}: từ giây ${startTime} đến ${startTime + currentDuration}`);
